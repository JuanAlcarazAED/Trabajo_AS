...
Otra alternativa es el “Set Partitioning in Hierarchical Trees”  (SPIHT) que a diferencia de los métodos tradicionales que tratan los coeficientes de manera aislada, SPIHT organiza los coeficientes en árboles de orientación espacial y clasifica los coeficientes por magnitud, transmitiendo primero los bits más significativos de los coeficientes más grandes.
...
A parte de los algoritmos ya mencionados, para dar una visión algo más amplia del campo de la compresión de imágenes de las herramientas disponibles hemos decidido incluir dos algoritmos más que también se basan en la transformada wavelet.

Estos dos algoritmos llamados EZW (_Embedded Zerotree Wavelet_) y SPIHT (_Set Partitioning in Hierachical Trees_) tienen la misma idea base, tomar la transformada wavelet de una imagen y hacer una selección de los coeficientes más importantes y aproximar su valor. Después esa información sobre la posición de los coeficientes con más información y el valor aproximado codificarla en una cola de _bits_, la cual ocupa mucho menos espacio que la imagen original, y gracias a ello puede transmitirse por cualquier canal o almacenarse más eficientemente. Con esa cola de _bits_ se puede usar un decodificador para extraer la información y recuperar la aproximada.
...
Sin dar muchos detalles, la idea básica consiste en darle una estructura de árbol a la matriz de coeficientes, se establece que el coeficiente en la posición $(i,j)$ tiene hijos, que son $(2i, 2j)$, $(2i+1, 2j)$, $(2i, 2j+1)$ y $(2i+1, 2j+1)$. Con este convenio se forma una estructura que permite analizar los coeficientes, pues en general si el coeficiente padre no es significativo, lo hijos es muy probable que no lo sean, aunque no siempre es cierto, esto se puede aprovechar para analizar todos los coeficientes de forma eficaz.

La diferencia principal entre ambos algoritmos es la forma en la que recorren la matriz de coeficientes, en ambos casos se aprovecha la estructura de árbol explicada. En EZW se recorre la matriz siguiendo un orden de barrido específico para no analizar nunca un coeficiente hijo antes que su padre, se sigue un barrido parecido a un _zigzag_; en SPIHT, que es una versión más refinada de EZW, se tienen tres listas LIS, LIP y LSP las cuales contienen índices de los coeficientes y siguiendo ciertas directrices se recorren las listas y se traspasan elementos de unas a otras o se añaden nuevos elementos en función de si los coeficientes son significativos o no.

Una explicación más detallada de ambos algoritmos se puede encontrar en el repositorio del proyecto, en el archivo `Explicacion_Algoritmos_EZW_SPIHT.rmd`.
...
Sobre el desempeño de la implementación realizada se puede ver con detalle en el archivo `Pruebas_SPIHT.Rmd`, pero en resumen se observa que el algoritmo agota todas las iteraciones posibles que puede hacer (que en el ejemplo son 7 y lo hace en un tiempo de entre 10 y 15 minutos, en el caso de la imagen de ejemplo empleada), por tanto en teoría la imagen se ha comprimido completamente y el decodificador debería obtener una imagen muy parecida a la original. Sin embargo nada más lejos de la realidad la imagen que se obtiene es de una calidad nefasta.
