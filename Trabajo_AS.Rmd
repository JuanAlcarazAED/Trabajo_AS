...
```{r}
compresion_SPIHT(imagen, J = 8, wf = "haar", max_bits = NULL, max_iter = 500){
  ...
  dwt_obj <- dwt.2d(img_gray, wf = wf, J = J)
...
```{r}
acantilado_re4<-resize_SPIHT(acantilado)
flores_re4<-resize_SPIHT(flores)
noche_re4<-resize_SPIHT(noche)
dibujo_re4<-resize_SPIHT(dibujo)

acantilado_gray <- grayscale(acantilado_re4)
flores_gray <- grayscale(flores_re4)
noche_gray <- grayscale(noche_re4)
dibujo_gray <- grayscale(dibujo_re4)
```
...
acantilado_dwt <- dwt.2d(acantilado_gray, wf = wf, J = J)
flores_dwt <- dwt.2d(flores_gray, wf = wf, J = J)
noche_dwt <- dwt.2d(noche_gray, wf = wf, J = J)
dibujo_dwt <- dwt.2d(dibujo_gray, wf = wf, J = J)

# Ensamblar matriz wavelet completa
W_acantilado <- Ensamblar(acantilado_dwt)
W_flores <- Ensamblar(flores_dwt)
W_noche <- Ensamblar(noche_dwt)
W_dibujo <- Ensamblar(dibujo_dwt)
```

```{r}

img <- grayscale(acantilado_re4)
