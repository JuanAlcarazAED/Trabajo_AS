...
```{r}
sep_audio_video(main="data/video2.mp4",audio="data/audio.wav",video="data/frames",fps=30)
```
...
La compresión de una señal mediante umbralizado de energía busca maximizar la dispersión lo que resulta en una distribución de coeficientes de baja entropía, que es ideal para la codificación final. Cuando se realiza la umbralización la mayoría de los coeficientes se agrupan en un determinado rango y el resto de valores pasan a ser muy pequeños, creando una distribución de coeficientes no uniforme con una distribución de baja entropía. 

El estudio de la entropía va a permitir ver el comportamiento de la compresión en función del parámetro umbral. Para tener en cuenta también la entropía máxima de la señal se va hacer el estudio desde la perspectiva de la redundancia de la señal. Escribimos la redundancia de la señal como:

$$R(\lambda)=1-\frac{H(\lambda)}{H_{max}(\lambda)}$$

Donde $H(\lambda)$ es la entropía de los coeficientes wavelet tras el threshold y $H_{max}$ es la entropía máxima posible que varía en función del valor umbral, ya que está definida como el logarítmo en base dos del número de posibles configuraciones, al aumentar el valor umbral las posibles configuraciones disminuyen. En este caso una redundancia alta significa que la señal está formada por pocos símbolos que dominan (muchos ceros y pocos valores no nulos), por lo tanto, una alta redundancia significa que la información está muy estructurada. 
