...
signal <- as.numeric(audio@left)
n.levels <- 6
wt <- dwt(signal, n.levels = n.levels)
lambda <- 0.1
...
compression_ratio <- n_original / n_kept
compression_ratio
```
...
hard_threshold <- function(coefs, lambda) {
  thr <- lambda * max(abs(coefs))
  coefs[abs(coefs) < thr] <- 0
  return(coefs)
}
...
wt_thr <- wt
wt_thr@W <- lapply(wt@W, function(Wi) hard_threshold(Wi, lambda))

reconstructed <- idwt(wt_thr)
reconstructed <- reconstructed[1:L]

# normalizar sin reventar
# reconstructed <- reconstructed / max(abs(reconstructed))

audio_comp <- Wave(right = reconstructed, samp.rate = fs, bit = 16)
writeWave(audio_comp, "data_comp/audio_comp2.wav")
