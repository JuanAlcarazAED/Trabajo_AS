...
Ahora, si recordamos la matriz de la transformada puede dividirse en cuatro zonas. Recordemos que al hacer la transformada hemos cambiado de dominio, de un dominio espacial hemos pasado al dominio frecuencia, y tenemos dos frecuencias, $\mu$ y $\nu$. Las cuatro zonas son HH(_High High_) valores de $\mu$ altos y de $\nu$ son altos, HL(_High Low_) valores de $\mu$ altos y de $\nu$ son bajos, LH(_Low High_) que sería al revés del anterior, LL(_Low Low_) donde ambas frecuencias son bajas.

Como se ha dicho antes, para una imagen la información más importante se encuentra en la zona LL de la matriz, las demás zonas también pueden contener información relevante aunque lo que contienen esos coeficientes wavelet está más relacionado con los detalles poco apreciables. Por tanto es posible prescindir de algunos de estos coeficientes sin perder mucha información de la imagen original, permitiendo comprimir.
...
Así con los coeficientes restantes haremos la transformada inversa y obtendremos una imagen que si elegimos bien los coeficientes que deseamos guardar y los que no, será casi igual a la original y ocupará mucho menos espacio.

El algoritmo EZW nos dirá cuales son los coeficientes que debemos guardar y la aproximación que hacemos de estos.

Para que este proceso sea eficiente la clave está en codificar la información. Lo cual consiste en guardar la información que hemos mencionado antes sobre los valores relevantes en forma de _bits_, que nos permitirá enviar la imagen por cualquier canal. Depués se decodificará la información a partir de la cual podremos reconstruir la imagen aproximada.
...
- Dado un coeficiente (que será un nodo del árbol) sus descendientes serán $4$. Si tenemos un coeficiente que se encuentra en la casilla $(i,j)$ de la matriz, sus descendientes directos son los coeficientes que se encuentran en las posiciones $(2i,2j)$, $(2i+1, 2j+1)$, $(2i+1, 2j)$ y $(2i, 2j+1)$. Es decir, el coeficiente $W[i,j]$ tiene como descendientes a los coeficientes $W[2i, 2j]$, $W[2i+1, 2j+1]$, $W[2i+1, 2j]$, $W[2i, 2j+1]$.

Si resulta que esos coeficientes no existen, porque las posiciones especificadas caen fuera de los límites, entonces el nodo sería un nodo hoja, sin hijos.

### Barrido de Morton

Es un método que sirve para recorrer todos los coeficientes wavelet de forma coherente con la estructura de padres e hijos que hemos explicado. De forma que un coeficiente no es analizado nunca antes que sus descendientes ni antes que sus padres.

Para verlo claramente tenemos la siguiente imagen:
