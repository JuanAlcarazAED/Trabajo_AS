...
```{r}
# img = img / max( img )
# img_final_baja = img_final_baja / max( img_final_baja )
20 * log10( max( img ) ) - 10 * log10( MSE( img, img_final_baja ) )
20 * log10( max( img ) ) - 10 * log10( MSE( img, img_final_alta ) ) 
```
...
file.size('data_comp/acantilado_DCT.jpg')
```
...
```{r}
ruta_acantilado <- 'im_2/acantilado.png'
acantilado<-load.image(ruta_acantilado)
save.image(acantilado, ruta_acantilado, quality = 1)
```
...
```{r}
file.size('im_2/acantilado.png')
file.size('data_comp/acantilado_IDWT.png')
file.size('data_comp/acantilado_DCT.png')
```

```{r}
print(object.size("acantilado"))
print(object.size("acantilado_IDWT"))
print(object.size("acantilado_DCT"))
```
...
```{r}
entropia(acantilado)
entropia(acantilado_IDWT)
entropia(acantilado_DCT)
```
...
```{r}
imagen_gris <- grayscale(acantilado)
v_imagen <- as.vector(imagen_gris)
h <- table(factor(v_imagen, levels = 0:255))
```
...
source("Codificacion_SPIHT_funcionals.R")
```
